...
6. Production evaluation must use real externally sourced skills only. Accept candidates whose `source.type` is `github-skill` and whose source identifies a real repository and skill body location. Never use `expert-call-seed`, `seed`, or `local` candidates as evidence that Expert Call improved a task. If the hosted API returns one, ignore it and report a registry-policy violation.
7. Do not describe a metadata-only routing card as a skill that was used. A candidate improves execution only when its external body or another concrete trusted procedure is actually loaded and operationalized.
